...
describe('End-to-end shorten then redirect flow', () => {
  it('shortens a URL and redirects to the original URL', async () => {
    ...
    const redirectResponse = await GET(new Request('http://localhost/' + code), makeCtx(code));
    ...
    const unknownResponse = await GET(new Request('http://localhost/unknown'), makeCtx('unknown'));
