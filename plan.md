...
- (재검증7) PKG-HEALTH standalone 가이드 후 첫 완주 확인
...
**PKG-HEALTH task 구조 (SCR-516 — 과부하 금지):**
- PKG-HEALTH 는 기능 task 에 **섞지 말고** 단일 standalone task(`depends_on`=모든 기능 task)로 두며, 그 task 의 유일한 일 = 통합 앱 표준 build+test 통과 확인(통합/lint 이슈만 수정, 기능 구현 X).
- **각 기능 task 는 그 자체로 lint·build clean**: unused var/import(예 빈 `catch (e)`) 금지 — node `next build` 는 전체 프로젝트에 ESLint 를 돌려 한 task 의 잔재가 통합 build 전체를 red 로 만든다(per-task 단위테스트는 통과해도). lint 를 PKG-HEALTH 로 미루지 말 것.
