...
acceptance:
  ...
  - id: A-2
    ...
    high_impact: true
  - id: A-3
    hint: "next.config 에 output:'standalone' 없음(Vercel 기본 타깃) + 추가 영속 의존성이 pnpm-lock 반영돼 pnpm install --frozen-lockfile && pnpm build 통과"
    high_impact: true
  - id: A-4
...
url-shortener 의 단축 링크를 serverless 영속 저장으로 바꾸고 Vercel 배포 호환으로 만든다 (publish 준비)

## 해야할 일

기존 url-shortener(main)의 in-memory Map store 를 영속 저장으로 교체 + Vercel 타깃 정합.
...
- 런타임 선택: 영속 백엔드 env(예 REST URL·TOKEN) 존재 시 영속, 없으면 in-memory (자격증명 하드코딩 금지)
- next.config: output:'standalone' 제거 (Vercel 기본 타깃)
- 추가 의존성을 package.json + pnpm-lock.yaml 에 반영 (frozen install 통과)
...
- A-3: next.config 에 output:'standalone' 없음(Vercel 기본 타깃) + 추가 영속 의존성이 pnpm-lock 반영돼 pnpm install --frozen-lockfile && pnpm build 통과
